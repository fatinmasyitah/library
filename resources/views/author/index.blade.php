                        ...
                        <tbody>
                            @foreach ($authors as $index => $item)
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{ asset($item->authorimage) }}" class="avatar" alt="{{ $item->authorname }}" width="60">
                                    </td>
                                    <td>{{ $item->authorname }}</td>
                                    <td>{{ $item->publisher->publishername }}</td>
                                    <td>
                                        <a class="btn btn-primary" href="{{ route('author.edit', $item->id) }}" data-toggle="tooltip" title="Edit"><i class="fas fa-pencil-alt"></i></a>
...
@section('scripts')
    <script>
        $('.delete').on('click', function () {
            const id = this.id;
            $('#deleteModal').attr('action', "{{ route('author.destroy', '') }}" + '/' + id);
        });
    </script>
